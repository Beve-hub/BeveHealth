import { ActionIcon, Avatar, Box, Button, Group, Image, rem, ScrollArea, Text, Textarea } from '@mantine/core';
...
const ChatField = ({ onUserSelect }: { onUserSelect: () => void }) => {
  ...
  return (
    <div style={{ 
      width: isSmallScreen ? '70rem' : '40rem',
     padding: isSmallScreen ? '0.1rem' : '0 2rem',
      borderTopRightRadius: '1rem',
       borderBottomRightRadius: '1rem',
       display: 'flex',
      flexDirection: 'column', }}>
      <Box style={{padding: isSmallScreen ? '0.5rem' : '1rem 2rem', display: 'flex', justifyContent: 'space-between' }}>
        <Group>
          {isSmallScreen && (
      ...
      <ScrollArea h={isSmallScreen ? "calc(100vh - 15rem)" : "60vh"} type="scroll" scrollbarSize={1} py={2}>
        <Box>
              ...
              <Group key={index} justify={message.sender === 'user' ? 'flex-end' : 'flex-start'}>
                {message.type === 'text' && (
                  <Text style={{
                    ...
                    backgroundColor: message.sender === 'user' ? '#008C73' : '#12121210',
      ...
      <Box style={{ padding:isSmallScreen ? '0rem' : '1rem ', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Group style={{display:'flex', backgroundColor: '#12121210', borderRadius: 20, padding: isSmallScreen ? '0.5rem' : '0.2rem 2rem' }}>
          <Textarea variant="unstyled" size="sm" placeholder="Type a message" style={{ width: isSmallScreen ? '10rem' : '20rem' }} />
          <Group>
        ...
        <Button variant='filled' color='#008C73' style={{ marginTop: isSmallScreen ? '0.5rem' : '0' }}>
          <IconSend />
        </Button>
      </Box>
    </div>
